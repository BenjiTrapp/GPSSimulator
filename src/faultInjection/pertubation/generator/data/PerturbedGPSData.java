package faultInjection.pertubation.generator.data;
 ...
 */
public final class PerturbedGPSData extends GPSData
{
	...
	{
		if (PerturbedGPSData.status != status)
			PerturbedGPSData.status = status;
	}
	...
	{
		if (!Objects.equals(PerturbedGPSData.latitude, latitude))
			PerturbedGPSData.latitude = latitude;
	}
	...
	{
		if (!Objects.equals(PerturbedGPSData.longitude, longitude))
			PerturbedGPSData.longitude = longitude;
	}
	...
	public synchronized static void setEW(CardinalDirections ew)
	{
		...
		if (PerturbedGPSData.ew != ew)
			PerturbedGPSData.ew = ew;
	}
	...
	public synchronized static void setNS(CardinalDirections ns)
	{
		...
		if (PerturbedGPSData.ns != ns)
			PerturbedGPSData.ns = ns;
	}
	...
	{
		if (!PerturbedGPSData.velocity.equals(velocity))
			PerturbedGPSData.velocity = velocity;
	}
	...
	{
		if (!PerturbedGPSData.altitude.equals(altitude))
			PerturbedGPSData.altitude = altitude;
	}
	...
	{
		if (PerturbedGPSData.course != course)
			PerturbedGPSData.course = course;
	}
	...
	public synchronized static void setSatellites(String  satellites)
	{
		...
		if (!PerturbedGPSData.satellites.equals(satellites))
			PerturbedGPSData.satellites = satellites;
	}
	...
	{
		if (PerturbedGPSData.quality != quality)
			PerturbedGPSData.quality = quality;
	}
	...
	{
		if (!PerturbedGPSData.pdop.equals(pdop))
			PerturbedGPSData.pdop = pdop;
	}
	...
	{
		if (!PerturbedGPSData.hdop.equals(hdop))
			PerturbedGPSData.hdop = hdop;
	}
	...
	{
		if (!PerturbedGPSData.vdop.equals(vdop))
			PerturbedGPSData.vdop = vdop;
	}
	...
	{
		if (PerturbedGPSData.mode != mode)
			PerturbedGPSData.mode = mode;
	}
	...
	{
		if (PerturbedGPSData.fixType != fixType)
			PerturbedGPSData.fixType = fixType;
	}
